/**
 * Apple FAQ Manager Module for Hotel 3 Paardekens Admin Panel
 * Handles CRUD operations for Apple FAQ blocks using Firebase Realtime Database
 */

(function() {
  'use strict';

  const STORAGE_KEY = 'hotel3p_apple_faq'; // Local fallback and migration key
  const FIREBASE_PATH = 'appleFaqBlocks'; // Firebase database path
  ...
  window.AppleFAQManager = {
    ...
    async createFAQBlock(blockData) {
      console.log('Creating new Apple FAQ block:', blockData?.en?.title || blockData?.title || 'untitled');
      const blocks = await this.getFAQBlocks();
     ...
     */
    _generateId() {
      return 'apple-faq-' + Date.now() + '-' + Math.random().toString(36).substring(2, 11);
    }
  };
